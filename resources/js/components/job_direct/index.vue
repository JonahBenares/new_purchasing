<script setup>
	...
	const getSupplier = async () => {
		if(jor_no.value != ''){
			let response = await axios.get("/api/djo_supplier_dropdown/");
			suppliers.value = response.data.suppliers;
			document.getElementById("djoi_supplier").disabled = false;
		}else{
			document.getElementById("djoi_supplier").disabled = true;
		}
	}

	const OpenBtn = async () => {
		if(vendor_details_id.value != ''){
			document.getElementById("generatejoibtn").disabled = false;
		}else{
			document.getElementById("generatejoibtn").disabled = true;
		}
	}
	

	const getAvailableJOR = async () => {
		let response = await axios.get("/api/get_direct_jorno/");
	...
	const generateJO = async () => {
		let response = await axios.get("/api/generate_direct_joi/"+jor_no.value+'/'+vendor_details_id.value);
		...
		joi_material_details.value.forEach(function (vals, index, theArray) {
			checkMaterialRemainingQty(vals.jor_material_details_id,vals.id,vals.quantity,index)
		});
	}
			...
			<div class="col-12 grid-margin stretch-card">
				<div class="card">
					<div class="card-body">
						<div class="row">							
							<div class="col-lg-8 offset-lg-2 col-md-3">
								<div class="form-group">
									<label class="text-gray-500 m-0" for="">Choose JOR No and Supplier</label>
									<input type="file" name="img[]" class="file-upload-default">
									<div class="input-group col-xs-12">
										<select class="form-control file-upload-info" v-model="jor_no" @change="getSupplier()">
											<option value="">--Select JOR Number--</option>
											<option :value="j.jor_no+'+'+j.id" v-for="j in jorno_dropdown" :key="j.jor_no+'+'+j.id">{{ j.jor_no }}</option>
										</select>
										<select class="form-control file-upload-info" id="djoi_supplier" v-model="vendor_details_id" @change="OpenBtn()" disabled>
											<option value="">--Select Supplier--</option>
											<option :value="sup.id" v-for="sup in suppliers" :key="sup.id">{{ sup.vendor_name }} ({{ sup.identifier }})</option>
										</select>
										<span class="input-group-append">
											<button class="btn btn-primary" type="button" id="generatejoibtn" @click="generateJO()" disabled>Select</button>
						...
						<div class="pt-1">
							<div v-if="(joi_head && joi_head.length!=0) || jor_head.length!=0 && (!allZeroLabor() || !allZeroMaterial())">
								<!-- <div v-show="jor_det"> -->
								<div class="row">
									<div class="col-lg-1">
												...
												<table class="table-bordered w-full !text-xs">
													...
													<tr class="" v-for="(jld,index) in joi_labor_details" v-if="props.id == 0">
														...
														<td class="border-y-none p-1" colspan="3">
															<input type="text" class="w-full bg-yellow-50 border-b p-1 loffer" :id="'loffer'+index" v-model="jld.offer">
														</td>
														...
														<td class="border-y-none p-1 text-right">
															<!-- {{jld.unit_price}} {{ jld.currency }} -->
															<input type="text" @keypress="isNumber($event)" @keyup="checkLaborBalance(jld.jor_labor_details_id,jld.id,vat,remaining_labor_balance[index], index)" :id="'unit_price'+index" class="w-full bg-yellow-50 border-b p-1 text-center" v-model="jld.unit_price">
															<select class="p-1 m-0 leading-none w-full text-center  bg-yellow-50" v-model="jld.currency">
													...
													<tr class="" v-for="(jml,indexes) in joi_material_details" v-if="props.id==0">
														...
														<td class="border-y-none p-1" colspan="2">
															<!-- {{jml.offer}} -->
															<input type="text" class="w-full bg-yellow-50 border-b p-1 moffer" :id="'moffer'+indexes" v-model="jml.offer">
														</td>
														<td class="border-y-none p-0 text-center">
														...
														<td class="border-y-none p-1 text-right">
															<!-- {{ jml.unit_price }} {{ jml.currency }} -->
															<input type="text" @keypress="isNumber($event)"  @keyup="checkMaterialBalance(jml.jor_material_details_id,jml.id,vat,remaining_material_balance[indexes], indexes)" :id="'unit_price'+indexes" class="w-full bg-yellow-50 border-b p-1 text-center" v-model="jml.unit_price">
															<select class="p-1 m-0 leading-none w-full text-center  bg-yellow-50" v-model="jml.currency">
																<option v-for="cur in currency" v-bind:key="cur" v-bind:value="cur">{{ cur }}</option>
															</select>
														</td>
														<td class="border-y-none p-1 text-right"><input type="text" class="text-center tmprice" @keypress="isNumber($event)" @keyup="checkMaterialBalance(jml.jor_material_details_id,jml.id,vat,remaining_material_balance[indexes], indexes)":id="'tmprice'+indexes" v-model="totalmprice" readonly></td>
													</tr>
