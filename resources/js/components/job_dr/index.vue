<script setup>
    ...
    let get_jo_alldr=ref([]);
    onMounted(async () => {
		getallDr()
	})
    const getallDr = async () => {
		let response = await axios.get("/api/get_jo_alldr");
		get_jo_alldr.value = response.data.drall;
	}
    // const data = [
                ...
                <div class="card">
                    <div class="card-body">
                        <div class="flex justify-between  mt-2 mb-0 absolute z-50 ">
                            <a href="/job_dr/new/0" class="btn btn-primary mt-2 mt-xl-0 text-white">
                                <span>Add New DR</span>
                            </a>
                        </div>
                        <div class=" pt-3">
                            <DataTable :data="get_jo_alldr" :options="options" class="display table table-bordered table-hover !border nowrap">
                                <thead>
                                    <tr>
                                        <th class="!text-xs bg-gray-100 uppercase text-center" width="5%"> DR Date</th>
                                        <th class="!text-xs bg-gray-100 uppercase text-center" width="20%"> DR No</th>
                                        <th class="!text-xs bg-gray-100 uppercase text-center" width="20%"> JOI No</th>
                                        <th class="!text-xs bg-gray-100 uppercase text-center" width="15%"> Mode</th>
                                        <th class="!text-xs bg-gray-100 uppercase text-center" width="1%" align="center"> 
                                ...
                                <template #column-4="props">
                                    <a :href="'/job_dr/view/'+props.rowData.id" class="btn btn-xs btn-warning text-white p-1">
