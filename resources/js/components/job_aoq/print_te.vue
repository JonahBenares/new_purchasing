<script setup>
	...
	let letters=ref([]);
	let laboroffers=ref([]);
	let vendor_aoq_labor=ref([]);
	let materialoffers=ref([]);
	let vendor_aoq_material=ref([]);
	let currency=ref([]);
	let count_rfq_vendors=ref(0);
	let jo_aoq_details_id=ref(0);
	...
	const getAOQHeadDetails = async () => {
		...
		jo_aoq_details_id.value = response.data.jo_aoq_details_id
	...
	const VendorItemsOffer = async () => {
		...
		const v = ven.split("_")
		let jo_rfq_vendor_id= v[0]
		
		let response = await axios.get('/api/jo_vendor_offers/'+jo_rfq_vendor_id+'/'+head.value.jo_rfq_head_id)
		vendor_aoq_labor.value = response.data.vendor_aoq_labor
		laboroffers.value = response.data.laboroffers
		vendor_aoq_material.value = response.data.vendor_aoq_material
		materialoffers.value = response.data.materialoffers
	...
	const SaveAdditionalVendor = () => {
		...
		const formAdditional= new FormData()
			...
			formAdditional.append('jo_aoq_head_id', props.id)
			formAdditional.append('jo_rfq_vendor_id', jo_rfq_vendor_id)

			formAdditional.append('laboroffers', JSON.stringify(laboroffers.value))
			formAdditional.append('materialoffers', JSON.stringify(materialoffers.value))
			axios.post("/api/add_jo_aoq_vendor", formAdditional).then(function () {
			closeModal()
				getAOQHeadDetails()
			});
	}
	...
	const DoneTE = () => {
		axios.post(`/api/done_te_jo_aoq/${props.id}`).then(function () {
			router.push('/job_aoq/view/'+props.id+'/'+jo_aoq_details_id.value)
		});
	}
				...
				<div class="">
					<table class="w-full !text-xs mb-3 ">
						...
						<tr>
							...
							<td class="">{{ head.project_activity }}</td>
						</tr>
					</table>
					<table class="table-bordered !text-xs mb-3" width="150%">
						...
						<!-- loop here if it is per item row (rowspan should not be equal to offers just add 1 (ie: 4-rowspan = 3-offers)) -->
						<template v-for="(md, m_index) in material_data">
							<tr>
								...
								<tr>
									...
									<template v-for="fo in first_offers">
										<template v-if="md.jor_material_details_id == fo.jor_material_details_id">
											...
											<td :class="(md.min_price == fo.unit_price) ? 'p-1 align-top bg-yellow-300' : 'p-1 align-top '">
												<div class="flex justify-between space-x-1">
								...
								<tr>
									...
									<template v-for="so in second_offers">
										<template v-if="md.jor_material_details_id == so.jor_material_details_id">
											...
											<td :class="(md.min_price == so.unit_price) ? 'p-1 align-top bg-yellow-300' : 'p-1 align-top '">
												<div class="flex justify-between space-x-1">
								...
								<tr>
									...
									<template v-for="to in third_offers">
										<template v-if="md.jor_material_details_id == to.jor_material_details_id">
											...
											<td :class="(md.min_price == to.unit_price) ? 'p-1 align-top bg-yellow-300' : 'p-1 align-top'">
												<div class="flex justify-between space-x-1">
				...
				<div class="col-lg-12" v-if="(head.status != 'Cancelled' && head.aoq_status != 'Awarded' && head.aoq_status != 'Done TE')">
					<div class="flex justify-between space-x-1">
						...
						<div class="flex justify-end space-x-1">
							<a :href="'/export-jo-aoq/'+head.jo_aoq_head_id" class="btn btn-primary ">Export</a>
					...
					<div class="modal_s_items">
						<div class="row">
							<div class="col-lg-12 col-md-3">
								...
								<div class="hidden" id="display_offers">
									...
									<table class="table-bordered w-full !text-xs" >
										<tr>
											<td class="p-1 text-center" width="5%">#</td>
											<td class="p-1 text-center" width="10%">JOR Qty</td>
										...
										<tr v-for="(vam, itemno) in vendor_aoq_material">
											<td class="p-1 align-top text-center">{{ itemno + 1 }}</td>
											<td class="p-1 align-top text-center">{{ parseFloat(vam.quantity).toFixed(2) }}</td>
											<!-- <td class="p-1 align-top text-center">{{ parseFloat(vam.remaining_qty).toFixed(2) }}</td> -->
											<td class="p-1 align-top">{{ vam.item_description }}</td>
											<td class="align-top">
												<div v-for="(mo, o) in materialoffers">
													<textarea type="text" class="border-b p-1 w-full h-14 !align-top offers_" :id="'offers'+ o" v-model="mo.offer" v-if="mo.jo_rfq_material_details_id == vam.jo_rfq_material_details_id"></textarea>
													<input type="hidden" class="materialofferid_" v-if="mo.jo_rfq_material_details_id == vam.jo_rfq_material_details_id" v-model="mo.jo_rfq_material_offer_id" >
												</div>
											</td>
											<td class="align-top">
											<div  v-for="mo in materialoffers">
												<div class="!h-14 border-b" v-if="mo.jo_rfq_material_details_id == vam.jo_rfq_material_details_id">
													<input type="text" class="border-b p-1 w-full !align-top text-center" placeholder="00.00" v-model="mo.unit_price" >
													<select class="p-1 m-0 leading-none w-full text-center  block text-xs whitespace-nowrap currency_" v-model="mo.material_currency">
														<option v-for="cur in currency" v-bind:key="cur" v-bind:value="cur">{{ cur }}</option>
													</select>
													<input type="hidden" class="materialofferid_" v-model="mo.jo_rfq_material_offer_id" >
												</div>
											</div>
											</td>
										</tr>
